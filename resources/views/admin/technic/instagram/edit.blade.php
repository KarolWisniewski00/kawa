                ...
                <div class="p-6 lg:p-8 bg-white border-b border-gray-200">
                    ...
                    <div class="flex flex-row justify-between">
                        <h1 class="mt-8 mb-4 text-2xl font-medium text-gray-900">
                            Edytowanie zdjęcia z instagrama
                        </h1>
                        <a href="{{route('dashboard.technic.instagram')}}" type="button" class="mt-8 mb-4 text-white bg-indigo-500 hover:bg-indigo-600 focus:ring-4 focus:ring-indigo-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 focus:outline-none"><i class="fa-solid fa-chevron-left me-2"></i>Powrót</span></a>
                    </div>
                    <form action="{{route('dashboard.technic.instagram.update', $instagram->id)}}" method="POST">
                        @method('PUT')
                        @csrf
                        <div class="mb-6">
                            <label for="text" class="block mb-2 text-sm font-medium text-gray-900">Link</label>
                            <input value="{{ old('url') ? old('url') : $instagram->url}}" name="url" type="text" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Tutaj wklej adres url do zdjęcia z instagrama" required>
                            @error('url')
                        ...
                        <div class="mb-6">
                            ...
                            <input value="{{ old('order') ? old('order') : $instagram->order}}" name="order" type="number" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Tutaj wklej adres url do zdjęcia z instagrama" required>
                            @error('order')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <ul class="grid w-full gap-6 md:grid-cols-3" id="photoGrid">
                                ...
                                <li class="{{ $index >= 9 ? 'hidden' : '' }}">
                                    ...
                                    checked
                                    @else 
                                    {{  $instagram->photo == $photo->getFilename() ? 'checked' : '' }}
                        ...
                        <a href="{{route('dashboard.technic.instagram')}}" class="text-red-500 hover:text-white border border-red-600 hover:bg-red-500 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2"><i class="fa-solid fa-x mr-2"></i>Anuluj</a>
                    </form>
                </div>
            </div>
        </div>
